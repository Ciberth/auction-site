...
router.delete('/:id', User.isLoggedIn, (req, res) => {
  Auction.findById(req.params.id, (err, auction) => {
    if(auction.bids[0].madeBy.toString() !== req.user._id.toString()) return res.status(400).send('You aren\'t the creator of this auction. Not authorized to delete.');
...
router.post('/:id/addBid', User.isLoggedIn, (req, res) => {
  Auction.findById(req.params.id, (err, auction) => {
    if(auction.bids[0].madeBy.toString() === req.user._id.toString()) return res.status(400).send('You can\'t place a bid on your own item.');
